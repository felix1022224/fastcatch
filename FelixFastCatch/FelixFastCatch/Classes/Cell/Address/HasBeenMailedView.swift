...
class HasBeenMailedView: UIView {
    ...
    func createProductGroup() -> () {
        ...
        // 設置每個 cell 的尺寸
        layout.itemSize = CGSize(width: CGFloat((UIScreen.main.bounds.width * 0.9 - 20) / 5 - 8), height: CGFloat((hasBeenProductBackground?.size.height)! + 5))
        ...
        productsGroup = UICollectionView(frame: CGRect(x: 10, y: timeLabel.bounds.height + 25, width: UIScreen.main.bounds.width * 0.9 - 20, height: (hasBeenProductBackground?.size.height)! * CGFloat(line) + 10 * CGFloat(line)), collectionViewLayout: layout)
...
//        mailedCodeLabel.isHidden = true
        ...
        contentBacngroundImage.frame = CGRect(x: 0, y: 0, width: UIScreen.main.bounds.width * 0.9, height: 10 * 2 + timeLabel.bounds.height + productsGroup.bounds.height + statusLabel.bounds.height + mailedTimeLabel.bounds.height +
