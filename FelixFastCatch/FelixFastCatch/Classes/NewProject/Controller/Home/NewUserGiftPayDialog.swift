...
class NewUserGiftPayDialog: UIView {
    ...
    func createView() {
        ...
        let attrText = NSMutableAttributedString.init(string: "确认支付 ￥\(1)")
        attrText.addAttribute(NSAttributedStringKey.font, value: UIFont.getCustomeYuanTiFont(fontSize: 16), range: NSRange(location: 5, length:String(1).count + 1))
            ...
            if response.error == nil && response.data != nil {
                ...
                if jsonData["code"].int! == 0 {
                    WeChatShared.pay(to: "main", jsonData["data"], resultHandle: { (result, identifier) in
                        switch(result){
                        case .Success:
                            ToastUtils.showSuccessToast(msg: "购买成功")
                            self.hide()
                            break;
                        case .Failed:
