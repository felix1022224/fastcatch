...
- (instancetype)initWithFrame:(CGRect)frame dataSource:(id<SPTabDataSource>)dataSource delegate:(id<SPTabDelegate>) delegate
{
    ...
    if (self) {
        ...
        self.backgroundView.frame = CGRectMake(7, self.bounds.origin.y, self.bounds.size.width - 14, self.bounds.size.height);
...
- (void)__setupTagBarView
{
    ...
    NSInteger offset = 0;
    NSInteger preferTabOffset = [self.tabDataSource respondsToSelector:@selector(preferTabLeftOffset)]?[self.tabDataSource preferTabLeftOffset]:-10;
    ...
    for (int i=0;i<[self.tabDataSource numberOfTab];i++) {
        ...
        [titleView.contentView addTarget:self action:@selector(pressTab:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)__setupMaskView
{
    if ([self needMarkView]) {
        ...
        self.markView.frame = CGRectMake(0, [self.tabDataSource respondsToSelector:@selector(markViewBottom)]?[self.tabDataSource markViewBottom]:-15, [self.tabDataSource markViewWidthForIndex:self.index], 3);
